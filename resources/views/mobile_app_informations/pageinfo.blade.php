                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-warning btn-sm editBtn" data-id="{{ $item->id }}"
                                    data-page_name="{{ $item->page_name }}" data-image="{{ $item->image_path }}"
                                    @foreach ($languages as $lang)
                                    data-title{{ $lang->code }}="{{ $item->translate('title', $lang->code) }}"
                                    data-description{{ $lang->code }}="{{ $item->translate('description', $lang->code) }}" @endforeach
        ...
        <div class="modal-dialog">
            <form method="POST" id="editForm" enctype="multipart/form-data" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <input type="text" name="page_name" id="edit_page_name" class="form-control mb-3" readonly>

                    @foreach ($languages as $lang)
                        <label>Title ({{ $lang->code }})</label>
                        <input type="text" name="title[{{ $lang->code }}]" id="edit_title_{{ $lang->code }}"
                            class="form-control mb-2" required>

                        <label>Description ({{ $lang->code }})</label>
                        <textarea name="description[{{ $lang->code }}]" id="edit_description_{{ $lang->code }}" rows="2"
                            class="form-control mb-3" required></textarea>
                    @endforeach

                    <label>Image</label>
                    <input type="file" name="image_path" class="form-control">
                    <div id="existingImageWrapper" class="mt-2" style="display:none;">
                        <p class="mb-1"><strong>Current Image:</strong></p>
                        <img id="existingImagePreview" src="" alt="Current Image" width="120"
                            class="border mb-2">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.editBtn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    // EXISTING IMAGE CONTROL
                    let imageWrapper = document.getElementById('existingImageWrapper');
                    let imgPreview = document.getElementById('existingImagePreview');
                    let removeCheck = document.getElementById('remove_image');
                    ...
                    document.getElementById('editForm').action =
                        "/mobile-app-information/page-info/" + id;
                });
            });
