...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            <h6 class="m-0 font-weight-bold text-primary">Breed List</h6>
        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @forelse($breads as $breed)
                            <tr id="breed-row-{{ $breed->id }}">

                                <td>{{ $breed->name }}</td>
                                <td>
                                    <button class="btn btn-sm btn-warning editBtn" data-id="{{ $breed->id }}"
                                        data-name="{{ $breed->name }}" data-bs-toggle="modal"
                                        data-bs-target="#editModal">Edit</button>
                                    <button class="btn btn-sm btn-danger deleteBtn"
                                        data-id="{{ $breed->id }}">Delete</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.deleteBtn').forEach(btn => {
                btn.addEventListener('click', () => {
                    ...
                    Swal.fire({
                        title: 'Are you sure?',
                        text: "This will delete the breed permanently!",
                        icon: 'warning',
                            ...
                            fetch(`/breads/delete/${id}`, {
                                ...
                                .then(data => {
                                    if (data.success) {
                                        ...
                                        let row = document.getElementById('breed-row-' +
                                            id);
                                        if (row) row.remove();
                                    }
                                });
